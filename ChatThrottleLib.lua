...
function ChatThrottleLib:SendAddonMessage(prio, prefix, text, chattype, target, queueName, callbackFn, callbackArg, logged)
	...
	if not self.bQueueing and nSize < self:UpdateAvail() then
		...
		if _G.C_ChatInfo then
			if logged then
				_G.C_ChatInfo.SendAddonMessageLogged(prefix, text, chattype, target)
			else
	...
	-- Message needs to be queued
	local msg = NewMsg()
	msg.f = _G.C_ChatInfo and (logged and _G.C_ChatInfo.SendAddonMessageLogged or _G.C_ChatInfo.SendAddonMessage) or _G.SendAddonMessage
	msg[1] = prefix
	msg[2] = text
	msg[3] = chattype
	msg[4] = target
